...
class imregpoc:
    ...
    def stitching(self, perspective=None):
        ...
        if xmin < 0:
            ref_start[0] = 0
            ref_end[0] = min(xmax, wid - 1)
            cmp_start[0] = xtrans
            cmp_end[0] = min(xmax + xtrans, xtrans + wid - 1)
        else:
            ref_start[0] = xmin
            ref_end[0] = min(wid - 1, xmax)
            cmp_start[0] = xmin
            cmp_end[0] = min(wid - 1, xmax)
        if ymin < 0:
            ref_start[1] = 0
            ref_end[1] = min(ymax, hei - 1)
            cmp_start[1] = ytrans
            cmp_end[1] = min(ymax + ytrans, ytrans + hei - 1)
        else:
            ref_start[1] = ymin
            ref_end[1] = min(hei - 1, ymax)
            cmp_start[1] = ymin
            cmp_end[1] = min(hei - 1, ymax)
        cmp_in = warpedimage[cmp_start[1]:cmp_end[1] + 1, cmp_start[0]:cmp_end[0] + 1]
        # ref 结果图， cmp 传入部分参考
        ref_new = fusion.imgFusion(self.orig_ref,cmp_in,ref_start,ref_end)
        # warpedimage[ytrans:ytrans + hei, xtrans:xtrans + wid] = ref_new
        warpedimage[ytrans:ytrans + hei, xtrans:xtrans + wid] = self.orig_ref
        # plt.figure()
        # plt.imshow(warpedimage, vmin=warpedimage.min(), vmax=warpedimage.max(), cmap='gray')
        # plt.show()
        return warpedimage, swidth, sheight, xtrans, ytrans
